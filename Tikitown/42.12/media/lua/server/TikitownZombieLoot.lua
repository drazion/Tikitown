...
local function baseballCardsCollection(zombie)
	if SandboxVars.Tikitown.CollectionBaseball then
		print("Rolling for Baseball Cards")
		local CommonCardChance = 300 -- 300/100,000 = 3/1,000 = 0.3% which is roughly 1 every 333 zombies killed
		local RareCardChance = 50 -- 0.05% drop chance, 1 in 1000
		...
		print("Baseball Cards Disabled")
		return
	end
end

local function rollForZombieLoot(zombie)
	baseballCardsCollection(zombie)	
	--print(SandboxVars.Tikitown.CollectionBaseball)
end
