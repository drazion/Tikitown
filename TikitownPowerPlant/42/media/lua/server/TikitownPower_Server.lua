...
local function isAdminOrDebug(player)
    ...
    return lvl and lvl ~= "" and lvl ~= "None"
end
...
-- just here to keep a handy save function.
...
local function applyDailyWear(data)
	...
	if not activated and ZombRand(0,100) > 50 then
		return
	end
...
local function sendSync(player)
    local data = TikitownPower.getData()
    print("[TikitownPower] sendSync, isServer=" .. tostring(isServer()))

    -- Always transmit moddata (handles MP clients via OnReceiveGlobalModData)
    ModData.transmit("TikitownPowerGrid")

    if isServer() then
        -- MP: push the full struct payload directly to the requesting client
        -- (or all clients if no specific player given)
        local payload = {
        ...
        -- SP: server and client share the same Lua environment, so
        -- ModData.transmit may not fire OnReceiveGlobalModData reliably.
        -- Call populate() directly
        print("[TikitownPower] sendSync SP path, refreshing UI directly")
        ...
        if TikitownPowerUI and TikitownPowerUI.instance then
            TikitownPowerUI.instance:syncParts(data)
        end
...
function TikitownPower.repairAllPartsTo100(player)
    local data = TikitownPower.getData()

    for structType, collName in pairs(STRUCT_COLLECTIONS) do
        local coll = data[collName] or {}
        local newColl = {}
            ...
            if type(struct) == "table" then
                ...
                for partID, cond in pairs(struct) do
                    if type(cond) == "number" then
                        local def = getPartDef(structType, partID)
                        local maxCond = (def and def.condition) or 100
                        newStruct[partID] = maxCond
                    else
                        newStruct[partID] = cond
                    end
                end
                newColl[key] = newStruct
            else
                newColl[key] = struct
            end
        end
    ...
    if player and player.Say then
        player:Say("All plant parts repaired to 100%")
    end
    sendSync(player)
end
...
Events.OnClientCommand.Add(function(modID, commandID, player, args)
    ...
    -------------------------------------------------
    -------------------------------------------------
    -- ADMIN/DEBUG: REPAIR ALL PARTS TO 100%
    -------------------------------------------------
    if commandID == "RepairAllParts" then
        if not isAdminOrDebug(player) then
            print("[TikitownPower] RepairAllParts denied: not admin/debug")
            return
        end

        TikitownPower.repairAllPartsTo100(player)
        return
    end
...
Events.EveryDays.Add(TikitownPower.calculateDays)
